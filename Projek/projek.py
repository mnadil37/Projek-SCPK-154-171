...
/* ── Profile Detail ──────────────────────── */
.profile-detail { font-size: 0.86rem; color: #374151; line-height: 2.1; }
...
# ── Konfigurasi Pool Kriteria (Dinamis) ────────────────────────────────────────
POOL_KRITERIA = {
...
def load_data():
    ...
    # Drop NA hanya untuk kolom numerik yang ada di dalam pool kriteria
...
if 'selected_models' not in st.session_state:
    # Default top 20 termurah untuk inisialisasi awal
    st.session_state.selected_models = list(df.nsmallest(20, 'Price')['Model'].unique())
if 'selected_kriteria' not in st.session_state:
    # Default 5 kriteria awal
    st.session_state.selected_kriteria = ['Price', 'Weight (inc. batteries)', 'Dimensions', 'Zoom wide (W)', 'Effective pixels']
...
# ══════════════════════════════════════════════════════════════
    ...
    # ── PANEL 1: Konfigurasi Kriteria & Alternatif ────────────────────────────
...
with st.expander(" Konfigurasi Sistem Pendukung Keputusan", expanded=True):
    ...
    # ✅ PERBAIKAN 1: Gunakan key unik untuk widget multiselect
    selected_kriteria = st.multiselect(
        ...
        default=st.session_state.get('selected_kriteria', []),  # ✅ Gunakan .get() untuk keamanan
        max_selections=5,
        key="kriteria_multiselect"  # ✅ Tambahkan key tetap
    )
    
    # ✅ PERBAIKAN 2: Update state hanya jika benar-benar berubah
    if selected_kriteria != st.session_state.get('selected_kriteria', []):
        st.session_state.selected_kriteria = selected_kriteria

    st.markdown("<br>**B. Pilih Alternatif Kamera**", unsafe_allow_html=True)
    col_btn1, col_btn2 = st.columns(2)
    
    with col_btn1:
        # ✅ PERBAIKAN 3: Gunakan on_click callback daripada langsung rerun
        if st.button("🔽 Top 20 Termurah", use_container_width=True, key="btn_termurah"):
            st.session_state.selected_models = list(df.nsmallest(20, 'Price')['Model'].unique())
            # Hapus st.rerun() - biarkan flow normal
    
    with col_btn2:
        if st.button("📸 Top 20 Resolusi Tinggi", use_container_width=True, key="btn_resolusi"):
            st.session_state.selected_models = list(df.nlargest(20, 'Effective pixels')['Model'].unique())
            # Hapus st.rerun() - biarkan flow normal
    ...
    with col_a1:
        ...
        # ✅ PERBAIKAN 4: Gunakan key dan on_change callback
        def on_models_change():
            """Callback saat pemilihan model berubah"""
            # State sudah otomatis terupdate via key
            pass
        ...
        selected_models = st.multiselect(
            ...
            key="models_multiselect",  # ✅ Key tetap
            on_change=on_models_change
        )
        
        # ✅ PERBAIKAN 5: Sync state dengan UI
...
if len(df_sel) < 2:
    st.error("⚠️ Pilih minimal 2 kamera sebagai alternatif untuk melanjutkan.")
    st.stop()
if len(selected_kriteria) != 5:
    st.warning("⚠️ Anda wajib memilih tepat 5 kriteria untuk melanjutkan perhitungan matriks 5x5.")
    st.stop()

    # ── PANEL 2: Perbandingan Berpasangan ────────────────────
    st.markdown('<div class="section-heading"><b>2.</b> Perbandingan Berpasangan Kriteria</div>', unsafe_allow_html=True)
    ...
    # 10 input berpasangan akan digenerate di sini karena kriteria tepat 5 (kombinasi 5C2 = 10)
    ...
    if run:
        ...
        with col_valid:
            ...
            <div class="info-box" style="padding: 1.2rem; text-align: center;">
                ...
                <div style="font-family: 'DM Mono', monospace; font-size: 2.2rem; font-weight: bold; color: {'#10b981' if CR <= 0.1 else '#ef4444'};">
                    {CR:.4f}
                </div>
                ...
                <p style="font-size: 0.8rem; color: #64748b; margin-top: 0.8rem;">
                    Syarat matematis: Nilai CR harus kurang dari atau sama dengan 0.1 (CR ≤ 0.1) agar pembobotan dianggap rasional dan dapat diandalkan.
                </p>
            </div>
            """, unsafe_allow_html=True)
        ...
        for i, k in enumerate(selected_kriteria):
            # Cek karakteristik dinamis (Cost / Benefit) dari dictionary POOL_KRITERIA
        ...
        # ── Chart 2: Radar-like Bobot Chart ─────────────
        ax2 = axes[1]
        ax2.set_facecolor('#0f1e38')

        # Tarik label nama pendek secara dinamis berdasarkan kriteria terpilih
        k_labels_short = [POOL_KRITERIA[k]['short'] for k in selected_kriteria]
        
        theta = np.linspace(0, 2 * np.pi, 5, endpoint=False)
        theta = np.concatenate([theta, [theta[0]]])
        ev_closed = np.concatenate([eigen_vector, [eigen_vector[0]]])

        ax2.remove()
        ax2 = fig.add_subplot(1, 2, 2, polar=True)
        ax2.set_facecolor('#0f1e38')

        ax2.plot(theta, ev_closed, color='#60a5fa', linewidth=2.5, zorder=3)
        ax2.fill(theta, ev_closed, color='#3b82f6', alpha=0.28)
        ax2.scatter(theta[:-1], eigen_vector, color='#f59e0b', s=60, zorder=5)

        for t, ev, lbl in zip(theta[:-1], eigen_vector, k_labels_short):
            ax2.text(t, ev + 0.02, f"{ev:.3f}", ha='center', va='bottom',
                     fontsize=8.5, color='#fbbf24', fontfamily='monospace', fontweight='bold')

        ax2.set_xticks(theta[:-1])
        ax2.set_xticklabels(k_labels_short, fontsize=9.5, color='#c8d6f0', fontweight='600')
        ax2.set_yticklabels([])
        ax2.grid(color='#1e3a5f', linewidth=0.8)
        ax2.spines['polar'].set_color('#1e3a5f')
        ax2.set_title(
            'Bobot Kriteria (Radar)',
            fontsize=12, fontweight='bold', color='#ffffff',
    ...
    with col_a:
        ...
        <div class="info-box">
            <div style="font-size:0.7rem;text-transform:uppercase;letter-spacing:0.1em;
                        color:#9ca3af;font-weight:600;margin-bottom:0.7rem;"> Metode</div>
            <div style="font-size:0.88rem;color:#374151;line-height:1.8;">
                Sistem ini menggunakan metode <b>Analytic Hierarchy Process (AHP)</b>
                untuk menentukan prioritas antar kriteria secara konsisten melalui
                perbandingan berpasangan skala Saaty.
            </div>
        </div>
        """, unsafe_allow_html=True)
    with col_b:
        ...
        <div class="info-box">
            ...
            <div style="font-size:0.88rem;color:#374151;line-height:2;">
                Pengguna dapat memilih tepat <b>5 kriteria</b> dari <b>12 atribut</b>
                kamera yang tersedia di dataset, lengkap dengan kategorisasi
                otomatis antara kriteria <i>Cost</i> dan <i>Benefit</i>.
            </div>
        </div>
        """, unsafe_allow_html=True)
